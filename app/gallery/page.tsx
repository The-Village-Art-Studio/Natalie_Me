                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {loading ? (
                            Array.from({ length: 6 }).map((_, i) => (
                                <div key={i} className="aspect-square rounded-2xl bg-black/[0.03] animate-pulse" />
                            ))
                        ) : artworks.length > 0 ? (
                            artworks.map((artwork, index) => (
                                <div
                                    ...
                                    className="group relative aspect-square overflow-hidden rounded-2xl bg-white/40 backdrop-blur-md border border-black/[0.06] shadow-sm transition-all duration-300 hover:border-black/[0.12] hover:scale-[1.02] cursor-pointer"
                                >
                                    <Image
                                    ...
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                                        ...
                                        <p className="text-white/80 text-xs font-light translate-y-2 group-hover:translate-y-0 transition-transform duration-300 delay-75">{artwork.medium}, {artwork.year}</p>
                                    </div>
                                </div>
                            ))
                        ) : (
                            <div className="col-span-full py-20 text-center">
                                <p className="text-stone-400 text-sm font-light">No artworks found in the gallery.</p>
                            </div>
                        )}
                    </div>
